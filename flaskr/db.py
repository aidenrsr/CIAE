...
def get_books():
    ...
    result = [{"book_name": book[0], "book_page_num": book[1]}
              for book in books]
...
def get_bookpage():
    ...
    cursor = db.cursor()
    cursor.execute(
        "SELECT book_id, page_num, page_text, page_pdf FROM book_page ")
...
def get_chapnum():
    ...
    cursor = db.cursor()
    cursor.execute(
        "SELECT chapter_id, book_id, start_page, end_page FROM chapter_page")
...
def get_scores():
    ...
    cursor = db.cursor()
    cursor.execute(
        "SELECT identification_score, catharsis_score, insight_score, score_total FROM score")
